    ...
    public function login()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                $stmt = $pdo->prepare('SELECT contraseña, Cargo_id_cargo FROM empleado WHERE Cedula = :cedula');
                ...
                if ($user) {
                    if ($password === $user['contraseña']) {
                        if ($user['Cargo_id_cargo'] == 1) {
                            // Redirigir al entorno de administrador
                            header('Location: ../views/admin/entorno_administrador.html');
                            exit();
                        } else {
